...
spec = describe "elf reader" $ do
  it "extracts all kernels from code object V3" $ do
    elf <-
      compileCo $ CodeObject {coCpu = "gfx90a", coMetadataVersion = 3, coKernels = [("test_kernel_a", "v_nop\ns_endpgm"), ("test_kernel_b", "s_endpgm")]}
    Right kernels <- readElf (DisasmTarget {disasmTriple = "amdgcn--amdhsa", disasmCPU = "gfx90a"}) elf
